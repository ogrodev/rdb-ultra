...
| Local KD-tree support index + AVX2 | `RINHIDX3` hour buckets with exact KD-tree branch-and-bound, release `x86-64-v3`, thin LTO, AVX2 distance dispatch | Leaf-8 local k6: p99 1.58/1.55/1.56ms, FP 0, FN 0, HTTP errors 0, scores 5801.34/5808.99/5806.20. Leaf-64 retune after official cache-miss regression: p99 1.60/1.48ms, FP 0, FN 0, HTTP errors 0, scores 5795.65/5829.32 |

Current best verified local run is now p99 1.48ms, FP 0, FN 0, HTTP errors 0, score 5829.32 with the `RINHIDX3` exact KD-tree support index plus AVX2 distance dispatch. First official preview test on the Mac mini Late 2014 (issue #2632) returned p99 12.89ms, FP 0, FN 0, HTTP errors 0, score 4889.76; structurally valid (mem=350, cpu=1, 2 API instances). The first KD-tree preview (issue #3098, leaf-8) returned p99 17.34ms, FP 0, FN 0, HTTP errors 0, score 4760.86, so the retained KD leaf size was widened to improve locality before the next official preview.
