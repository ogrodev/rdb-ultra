...
Current best verified local run remains the hour-bucket support KNN path at p99 1.75ms, FP 0, FN 0, HTTP errors 0, score 5757.64. Reaching 5900+ with weighted errors 0 requires p99 below about 1.26ms. The attempted sparse-grid, parser, bounded-distance, release-profile, mmap-prefault, and `maxconn 128` refactors did not improve local score under current constraints. The retained repo state is the hour-bucket/mmap/maxconn-256 path; later local reruns reproduced zero detection errors but showed p99 noise rather than the historic 1.75ms.
