...
Current best verified local run is now p99 1.76ms, FP 0, FN 0, HTTP errors 0, score 5754.18 — within rounding noise of the historic 5757.64. First official preview test on the Mac mini Late 2014 (issue #2632) returned p99 12.89ms, FP 0, FN 0, HTTP errors 0, score 4889.76; structurally valid (mem=350, cpu=1, 2 API instances) but ~7x higher p99 than local M4 measurements, consistent with the older 2.6 GHz Intel CPU. Reaching 5900+ with weighted errors 0 requires p99 below about 1.26ms; on the official runner that would mean reducing per-request scan work itself rather than tuning the local Docker stack. The retained repo state is the hour-bucket/mmap/maxconn-256 path with `HourBucketIndex::warmup` at startup and HAProxy `timeout http-keep-alive 60s`.
